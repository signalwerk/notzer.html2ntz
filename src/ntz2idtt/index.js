      ...
      _.forEach(rows, (item) => {
        ...
        if(row.processor && row.processor.type === 'tr' ) {
          ...
          _.forEach(row.children, (cell, iteratee) => {
              ...
              table.processor.colWidth[iteratee] = Math.max(table.processor.colWidth[iteratee], cell.style.width);
          });
          ...
          table.processor.columnCount = Math.max(table.processor.columnCount, rowCells.length);

        } else {
          console.log('error! table child is not a tr!', row.length, JSON.stringify(row) );
